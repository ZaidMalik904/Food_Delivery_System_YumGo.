    ...
    return (
        <div className="navbar sticky top-0 z-50 bg-white flex justify-between items-center py-5 px-[5vw] shadow-sm">
            ...
            <div className="navbar-right flex items-center gap-4 md:gap-10">
                ...
                {!token ?
                    <button onClick={() => setShowLogin(true)} className='hidden md:block bg-transparent text-[18px] font-bold text-black border border-[tomato] py-2.5 px-7 rounded-[50px] hover:bg-[#fff4f2] transition duration-300'>Sign In</button>
            ...
            {isMenuOpen && (
                <div className="absolute top-[80px] left-0 w-full bg-white shadow-lg flex flex-col items-center gap-6 py-6 md:hidden animate-slideDown z-40 border-t border-gray-100">
                    <a href='/#home' onClick={() => { setMenu("home"); setIsMenuOpen(false); }} className={menu === "home" ? "text-[tomato] font-bold text-xl" : "text-black font-medium text-xl hover:text-[tomato]"}>Home</a>
                    <a href='/#explore-menu' onClick={() => { setMenu("menu"); setIsMenuOpen(false); }} className={menu === "menu" ? "text-[tomato] font-bold text-xl" : "text-black font-medium text-xl hover:text-[tomato]"}>Menu</a>
                    <a href='/#about-us' onClick={() => { setMenu("about-us"); setIsMenuOpen(false); }} className={menu === "about-us" ? "text-[tomato] font-bold text-xl" : "text-black font-medium text-xl hover:text-[tomato]"}>About Us</a>
                    <a href='/#footer' onClick={() => { setMenu("contact-us"); setIsMenuOpen(false); }} className={menu === "contact-us" ? "text-[tomato] font-bold text-xl" : "text-black font-medium text-xl hover:text-[tomato]"}>Contact Us</a>

                    {!token ? (
                        <button onClick={() => { setShowLogin(true); setIsMenuOpen(false); }} className='bg-[tomato] text-white text-lg font-bold py-2 px-8 rounded-full shadow-md hover:bg-[#e45b41] transition w-[80%]'>Sign In</button>
                    ) : (
                        <div className='flex flex-col items-center gap-4 w-full'>
                            <button onClick={() => { navigate('/my-orders'); setIsMenuOpen(false); }} className='flex items-center gap-2 text-black font-medium hover:text-[tomato]'><ShoppingCart className='w-5 h-5' /> Orders</button>
                            <button onClick={() => { logout(); setIsMenuOpen(false); }} className='flex items-center gap-2 text-black font-medium hover:text-[tomato]'><LogOut className='w-5 h-5' /> Logout</button>
                        </div>
                    )}
                </div>
            )}
